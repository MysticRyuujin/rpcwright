...
- **hive checks the spec only for `speconly`** (#9, hive.md): ordinary tests replay the `.io` fixture exact-match, so changing the spec without regenerating the fixture does nothing; `speconly` tests validate against the OpenRPC result schema, which the sim builds from spec source via specgen (no structural fallback).
...
- **An open result schema disagrees with exact-match tests** (#14, execution-apis.md): `required` forces presence, but only `additionalProperties: false` forbids extras — without it, a client's extra field is spec-valid yet fails hive. Default new result schemas to closed; know the fork-versioning cost and the `allOf` trap.
...
- **The shipped chain has every fork boundary — pre-fork behavior IS testable** (hivechain.md): Berlin activates at block 24, Cancun at 42, Prague at 45 (`tests/genesis.json`), so "field X at a block before its fork" is a block-number param away (`"0x14"`). Do not claim "untestable on the shared chain." For a six-client behavior survey, drop placeholder `.io` files in the sim tests dir and read the raw `<<` lines from `workspace/logs/details/` — one 2-minute run beats any static code trace.
...
- **Known standing client bugs — don't re-discover these** (#13, clients.md): reth can't produce legacy pre-Byzantium receipts (always emits the Byzantium+ status byte); ethrex can't import the hive chain past its point of pre-merge support, so it has real historical-state gaps on the stock chain. Both resurface on unrelated changes — recognize them, don't re-investigate.
